...
### Вопрос 2.3: Поддержка клавиатуры
**Вопрос:** Должна ли быть поддержка клавиатуры для управления игрой?

**Предположительный ответ:**
- **Поддержка клавиатуры:** отсутствует
- Управление осуществляется только через мышь (десктоп) или касание (мобильные)
- Клавиатурные сокращения не реализуются

### Вопрос 2.4: Локализация и интернационализация
...
- **Формат чисел:** ISO 8601 для дат, ISO 80000 разделители для чисел
...
### Вопрос 2.5: Визуальное отображение и цветовые схемы
...
### Вопрос 2.6: Сохранение состояния игры
...
- **Ранее сохранённые игры:**
  ...
  - **Кнопка удаления в списке:** рядом с каждым элементом сохранения расположена кнопка удаления с иконкой удаления (значок корзины/мусорной бочки)
    - При клике на кнопку удаления выскакивает попап с подтверждением
    - Две кнопки в попапе: "Отмена" (закрывает попап) и "Удалить" (удаляет сохранение из localStorage)
  - **Количество сохранений:** максимальное количество определяется доступным пространством localStorage (пользователь сам управляет удалением старых сохранений)
  - **Может быть несколько незавершённых игр одновременно**
- **Ограничения по символам:**
  - **Название забега:** максимум 256 символов
  - **Имя пользователя:** максимум 128 символов
- **Очистка:** при завершении игры (победа/поражение) текущее состояние удаляется, но статистика сохраняется

### Вопрос 2.7: Анимация и визуальные эффекты
...
- **Звуковые эффекты:** отсутствуют (нет audio-сопровождения)
...
### Вопрос 3.5: Ограничение по времени игры
**Вопрос:** Есть ли максимальное ограничение по времени игры?

**Предположительный ответ:**
- **Максимальное время игры зависит от уровня сложности:**
  - **Новичок:** до 10 минут (максимум 600 секунд)
  - **Любитель:** до 40 минут (максимум 2400 секунд)
  - **Профессионал:** до 99 минут (максимум 5940 секунд)
- **Условие:** игра может быть завершена раньше при победе или поражении, но не может продолжаться дольше установленного максимума для выбранного уровня
- **Таймер:** отсчитывает время в секундах и отображает в формате mm:ss

### Вопрос 3.6: Сохранение и восстановление состояния игры
**Вопрос:** Как должно реализоваться автоматическое сохранение состояния игры?

**Предположительный ответ:**
- **Автоматическое сохранение:** состояние текущей игры записывается в localStorage браузера после каждого хода (ответ на техническую реализацию см. в вопросе 2.6)
- **Сохраняемые данные:**
...
### Вопрос 3.7: Параметры уровней сложности
...
### Вопрос 3.8: Пользовательский уровень сложности
